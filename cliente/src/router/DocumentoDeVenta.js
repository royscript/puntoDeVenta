...
const DocumentoDeVenta = ({children, logOut, conseguirPermisos, usuario})=>{
    ...
    const listarDocumentoVenta = async (search)=>{
        try {
            const resultSet = await axios.post('/documento-de-venta/listar', {pagSiguiente : pagSiguiente, cantPorPag : cantPorPag, search});
    ...
    const eliminarDocumentoVenta = async (id)=>{
        try {
            const resultSet = await axios.post('/documento-de-venta/eliminar', {id});
            setRespuestaConsulta(
                <div className="alert alert-danger" role="alert">
    ...
    return(
        ...
        <NavBar usuario={usuario.nombreUsuario+" "+usuario.apellidoUsuario} logOut={logOut}/>
            <div className="container-fluid">
                <div className="row">
                    <SidebarMenu seccion={"documento-de-venta"}/>
                    <main className="col-md-9 ms-sm-auto col-lg-10 px-md-4">
                        <HeaderContainer titulo={titulo}/>
                        {children}
                        <Formik
                            initialValues={valoresFormulario || {idDocumentoDeVenta : '', nombreDocumentoDeVenta : ''}}
                            enableReinitialize
                            validate={
                                (values) => {
                                    const errors = {}
                                    if(!values.nombreDocumentoDeVenta) {
                                        errors.nombreDocumentoDeVenta = 'Requerido'
                                    } else if (values.nombreDocumentoDeVenta.length < 5) {
                                        errors.nombreDocumentoDeVenta = 'Ingresa el Nombre del Tipo de Venta'
                                    }
                                    return errors
                                }
                            }
                            ...
                            onSubmit={async (values,{resetForm,submitForm})=>{
                                if(botonPresionado==="Guardar"){
                                    axios.put('/documento-de-venta/insertar', { nombreDocumentoDeVenta : values.nombreDocumentoDeVenta })
                                        .then(res => {
                                            if(res.status===200){
                                                resetForm({values: ''});
                                    ...
                                    axios.put('/documento-de-venta/editar', { nombreDocumentoDeVenta : values.nombreDocumentoDeVenta,
                                        idDocumentoDeVenta : values.idDocumentoDeVenta })
                                    .then(res => {
                                        if(res.status===200){
                                            resetForm({values: ''});
                                            setValoresFormulario(null);
                        ...
                        >
                            ...
                            <div className="accordion" id="accordionExample">
                                    ...
                                    <div className="accordion-body">
                                        <div className="card" style={{"width": "100%"}}>
                                            <div className="card-body">
                                                <InputReadOnly name="idDocumentoDeVenta" label="Id Documento de Venta"/>
                                                <Input name="nombreDocumentoDeVenta" label="Nombre Documento de Venta" type="text"/>
                                            </div>
                                ...
                                head={
                                    <tr>
                                        <th>#</th>
                                        <th>Nombre Documento de Venta</th>
                                        <th>Accion</th>
                                    </tr>
                                }
                                mostrarDatos={
                                    (value,index)=>
                                    <tr key={index+'fila'}>
                                        <td>{value.idDocumentoDeVenta}</td>
                                        <td>{value.nombreDocumentoDeVenta}</td>
                                        <td>
                                            <div className="btn-group" role="group">
                                                ...
                                                <button type="button" className="btn btn-danger" onClick={
                                                    ()=>{
                                                        var resp = window.confirm(`Desea eliminar esta familia? ${value.nombreDocumentoDeVenta}`);
                                                        if(resp) eliminarDocumentoVenta(value.idDocumentoDeVenta);
                                                    }
                                                }>
...
export default DocumentoDeVenta;